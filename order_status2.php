...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<input type="hidden" name="order-id" value="' . $orderID . '">';
        echo '<input type="hidden" name="status-toggle" value="' . $status . '">';
...
<script>
    ...
    toggleButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            const orderIDInput = button.querySelector('input[name="order-id"]');
            ...
            xhr.send('order-id=' + encodeURIComponent(orderID) + '&status=' + encodeURIComponent(status));
        });
    });
</script>
